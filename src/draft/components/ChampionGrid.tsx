...
import { type Champion, type ChampionRole, getChampions, filterChampions, getChampionImageUrl } from '../services/championService'
...
const POSITION_ICONS_BASE_URL = 'https://raw.communitydragon.org/latest/plugins/rcp-fe-lol-clash/global/default/assets/images/position-selector/positions'
...
export function ChampionGrid({
  ...
  useEffect(() => {
    ...
    if (selectedRole) {
      filtered = filtered.filter(champion => champion.roles.includes(selectedRole))
    }
  ...
  return (
    <div className='h-full flex flex-col space-y-2'>
        ...
        <div className='flex gap-1'>
          {Object.entries(roleIcons).map(([role, iconUrl]) => (
            <Button
              ...
              onClick={() => setSelectedRole(selectedRole === role ? null : role as ChampionRole)}
            >
              <img 
                src={iconUrl} 
                alt={role}
                className={selectedRole === role ? 'brightness-125' : 'opacity-75 hover:opacity-100'}
              />
            </Button>
          ))}
        </div>
      ...
      {/* Grid */}
      <div className='min-h-0 grid grid-cols-8 gap-4 overflow-y-auto p-6'>
