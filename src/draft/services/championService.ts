...
export async function getChampions(): Promise<Champion[]> {
  ...
  try {
    ...
    await Promise.all(championsCache.map(async champion => {
      try {
        const response = await fetch(`${COMMUNITY_DRAGON_URL}/v1/champions/${champion.key}.json`)
        ...
        console.error(`Failed to fetch splash path for ${champion.name}:`, error)
      }
    }))
...
export function getChampionImageUrl(champion: Champion | string, type: 'icon' | 'splash' = 'icon'): string {
